...
def _dist_to_opening(env:ManagerBasedRLEnv, asset: Articulation) -> torch.Tensor:
    return torch.norm(offset_to_opening_env(env, asset), dim=-1)
...
def heading_to_opening_exp(env, std=0.8, asset_cfg=SceneEntityCfg("robot")):
    asset = _get_asset(env, asset_cfg)
    delta = offset_to_opening_env(env, asset)
...
def height_error_to_opening_l2(env, asset_cfg=SceneEntityCfg("robot")):
    asset = _get_asset(env, asset_cfg)
    z_err = robot_position_env(env, asset)[:, 2] - get_opening_targets(env)[:, 2]
